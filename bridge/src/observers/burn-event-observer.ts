...
export class EthereumBurnEventObserver implements IObserver<{ blockHash: BlockHash, events: (EventData & TransactionLocation)[] }> {
    ...
    constructor(ncgTransfer: INCGTransfer, slackMessageSender: ISlackMessageSender, monitorStateStore: IMonitorStateStore, explorerUrl: string, etherscanUrl: string, integration: Integration) {
