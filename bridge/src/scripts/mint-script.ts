...
const EXISTING_SAFE_ADDRESS = ethers.utils.getAddress(process.env.SAFE_ADDRESS!)
...
async function proposeMintTransaction(amount: string, to: string, gasPrice: number) {
...
async function main(destination: string,  amount: string) {
    ...
    if (amountToNum > Number(process.env.MAX_TRANSFER_AMOUNT) || amountToNum === 0){
    ...
    console.log('===========================================================================================================')
    console.log(`Are you trying to Transfer WNCG. To: ${destination}, Amount: ${amount} ??`)
    ...
    await reader.on('line', async (input) => {
        ...
        const safeTxHash = await proposeMintTransaction(amount, ethers.utils.getAddress(recipient), gasPrice);
        const { safeTxHash: confirmedSafeTxHash } = await confirmTransaction(safeTxHash)
