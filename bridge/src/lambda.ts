...
import { DynamoMonitorStateStore } from "./dynamo-monitor-state-store";
...
import { DynamoExchangeHistoryStore } from "./dynamo-exchange-history-store";
...
import {
    ...
    relayNineChronicles,
    EthereumRelayerDeps,
    NineChroniclesRelayerDeps,
} from "./relayer";

async function initializeDeps() {
    ...
    const BSC_KMS_PROVIDER_URL: string = Configuration.get(
        "BSC_KMS_PROVIDER_URL"
    );
    const BSC_WNCG_CONTRACT_ADDRESS: string = Configuration.get(
        "BSC_WNCG_CONTRACT_ADDRESS"
    );
    ...
    const monitorStateStore: IMonitorStateStore =
        DynamoMonitorStateStore.create();
    const exchangeHistoryStore: IExchangeHistoryStore =
        DynamoExchangeHistoryStore.create();
    ...
    const bscWNCGToken: ContractDescription = {
        abi: wNCGTokenAbi,
        address: BSC_WNCG_CONTRACT_ADDRESS,
    };
    ...
    const bscBurnEventObserver = new EthereumBurnEventObserver(
        ncgKmsTransfer,
        slackMessageSender,
        opensearchClient,
        spreadsheetClient,
    ...
    const ethDeps: EthereumRelayerDeps = {
        provider,
        contractDescription: wNCGToken,
        monitorStateStore,
        burnEventObserver: ethereumBurnEventObserver,
        networkKey: "ethereum",
    };

    const bscDeps: EthereumRelayerDeps = {
        provider: bscProvider,
        contractDescription: bscWNCGToken,
        monitorStateStore,
        burnEventObserver: bscBurnEventObserver,
        networkKey: "bsc",
    };

    const ncDeps: NineChroniclesRelayerDeps = {
        headlessGraphQLClient: headlessGraphQLCLient,
        monitorStateStore,
    ...
    return {
        pendingTransactionRetryHandler,
        ethDeps,
        bscDeps,
        ncDeps,
    };
...
export async function handler(): Promise<void> {
    ...
    const { pendingTransactionRetryHandler, ethDeps, bscDeps, ncDeps } =
